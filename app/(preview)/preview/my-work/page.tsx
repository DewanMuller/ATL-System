...
export default async function PreviewMyWorkPage() {
    ...
    o.keyResults.flatMap((kr) =>
      kr.initiatives
        ...
        .map((i) => ({
          ...
          owner: {
            initials: i.responsibleUser.name
              ? i.responsibleUser.name.trim().split(/\s+/).map((p) => p[0]).slice(0, 2).join("").toUpperCase()
              : i.responsibleUser.email.slice(0, 2).toUpperCase(),
            name: i.responsibleUser.name || i.responsibleUser.email,
          },
          comments: i.comments,
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {ledObjectives.map((o) => {
              ...
              return (
                <Link
                  key={o.id}
                  href={`/preview/okr-workspace/${o.id}`}
